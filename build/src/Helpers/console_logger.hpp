...
// Singleton debug console logger with OPTIMIZED rate limiting
class ConsoleLogger {
private:
    ...
    static int logCount;
    static const int MAX_LOGS_PER_SECOND = 5; // REDUCED from 10 to prevent flooding
    static std::chrono::steady_clock::time_point lastResetTime;
    
    // Frame-based rate limiting for high-frequency logs
    static int frameCounter;
    static const int LOG_EVERY_N_FRAMES = 30; // Only log every 30 frames for frequent events
    ...
    static void createDebugConsole() {
        ...
        DWORD written;
        const char* header = "========================================\n  SUBARO DEBUG CONSOLE (OPTIMIZED)\n========================================\nMax 5 logs/sec, frame-limited updates\nPress [0] in main console to toggle.\n\n";
...
public:
    ...
    // TRIGGERBOT LOGGING (OPTIMIZED)
    static void logTriggerUpdate(bool hasTarget, uint32_t crosshairID) {
        if (!shouldLogFrame()) return; // Frame-based rate limiting
    ...
    static void logTriggerTargetTeammate(uint8_t team) {
        // Only log teammate detection occasionally
        static int teammateSkipCount = 0;
        if (++teammateSkipCount % 60 != 0) return;
        if (!debugEnabled) return;
        
        char buffer[256];
        snprintf(buffer, sizeof(buffer), "[TB:%s] Teammate detected (Team %d) - Skipped x%d\n",
                 getTimestamp().c_str(), (int)team, teammateSkipCount);
    ...
    static void logTriggerNoTarget(const char* reason) {
        ...
        // Log every 300 frames instead of 100
        if (skipCount % 300 != 0) return;
        if (!debugEnabled) return;
    ...
    static void logBhopMemoryWrite(std::uintptr_t address, uint32_t value, const char* action) {
        // Only log every 10th jump to avoid spam
        static int jumpWriteCount = 0;
        if (++jumpWriteCount % 10 != 0) return;
        if (!shouldLog()) return;
        
        char buffer[256];
        snprintf(buffer, sizeof(buffer), "[BH:%s] %s - Addr: 0x%llX | Value: %u (x%d)\n",
                 getTimestamp().c_str(), action, (unsigned long long)address, value, jumpWriteCount);
    ...
    static void logInfo(const char* feature, const char* message) {
        if (!shouldLog()) return; // Rate limit info messages
