...
namespace OffsetUpdater {
    ...
    // Download a2x/cs2-dumper offsets.hpp
    inline bool DownloadOffsetsHpp(std::string& offsets_hpp) {
        try {
            offsets_hpp = DownloadFile(
                "https://raw.githubusercontent.com/a2x/cs2-dumper/main/output/offsets.hpp"
            );
            
            if (offsets_hpp.empty() || offsets_hpp.find("namespace") == std::string::npos) {
                return false;
            }
    ...
    // Download a2x/cs2-dumper buttons.hpp
    inline bool DownloadButtonsHpp(std::string& buttons_hpp) {
        try {
            buttons_hpp = DownloadFile(
                "https://raw.githubusercontent.com/a2x/cs2-dumper/main/output/buttons.hpp"
            );
            
            if (buttons_hpp.empty() || buttons_hpp.find("namespace") == std::string::npos) {
                return false;
            }
    ...
    inline bool DownloadClientDllHpp(std::string& client_dll_hpp) {
        try {
            // Try sezzyaep's version first (has more offsets)
