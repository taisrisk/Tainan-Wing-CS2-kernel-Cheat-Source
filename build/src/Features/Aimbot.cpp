...
// ===== OPTIMIZED: FIND CLOSEST TARGET - FIXED FOV AND PRIORITY =====
const Entity* Aimbot::findClosestTarget(const EntityManager& entities) {
    ...
    for (const auto& enemy : targetList) {
        ...
        // FIXED: Distance from center (PRIMARY METRIC)
        float dx = screenPos.x - screenCenter.x;
        float dy = screenPos.y - screenCenter.y;
        float distanceFromCenter = std::sqrt(dx * dx + dy * dy);

        // FIXED: FOV_RADIUS is 600 pixels (not combined with 3D distance)
        if (distanceFromCenter > FOV_RADIUS) continue;

        // NEW: Calculate angle to target (prefer targets player is already looking at)
        Vector3 direction;
        direction.x = currentPos.x - localPos.x;
        direction.y = currentPos.y - localPos.y;
        direction.z = currentPos.z - localPos.z;
        ...
        float targetPitch = -std::asin(direction.z / dist3D) * (180.0f / M_PI);
        float targetYaw = std::atan2(direction.y, direction.x) * (180.0f / M_PI);
        ...
        float angleDifference = std::sqrt(pitchDiff * pitchDiff + yawDiff * yawDiff);
        
        // FIXED: Priority = Screen distance (80%) + Angle preference (20%)
        // Targets player is already looking at get priority
        float angleBonus = angleDifference / 90.0f; // Normalize to 0-1 range (90 degrees max)
        float finalScore = (0.8f * distanceFromCenter) + (0.2f * angleBonus * 600.0f); // Scale angle to screen pixels

        // FIXED: Use SCREEN DISTANCE as primary selection criteria
