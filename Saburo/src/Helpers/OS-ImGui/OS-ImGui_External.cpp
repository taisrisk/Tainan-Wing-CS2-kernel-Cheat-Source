...
namespace OSImGui
{
    ...
    void RegisterRawInput(HWND hwnd) {
		...
		std:printf("Failed to register raw input device. Error: %d\n", GetLastError());
        }
    }
    ...
    void OSImGui_External::AttachAnotherWindow(std::string DestWindowName, std::string DestWindowClassName, std::function<void()> CallBack)
    {
        ...
        Window.Name = "NovazBesting Kernel";
        Window.wName = StringToWstring(Window.Name);
        Window.ClassName = "WindowClass";
        Window.wClassName = StringToWstring(Window.ClassName);
    ...
    void OSImGui_External::MainLoop()
    {
        ...
        const std::chrono::microseconds targetFrameTime(8333); // 120 FPS = ~8.3ms per frame
        ...
        while (!done)
        {
            ...
            // Update window data if attached to another window
            if (Type == ATTACH)
            {
                if (!UpdateWindowData())
                {
            ...
            const float clear_color[4] = { 
                Window.BgColor.Value.x, 
                Window.BgColor.Value.y, 
                Window.BgColor.Value.z, 
                Window.BgColor.Value.w 
            };
            
            g_Device.g_pd3dDeviceContext->OMSetRenderTargets(1, &g_Device.g_mainRenderTargetView, nullptr);
            g_Device.g_pd3dDeviceContext->ClearRenderTargetView(g_Device.g_mainRenderTargetView, clear_color);
            
            ImGui_ImplDX11_RenderDrawData(ImGui::GetDrawData());

            // Present WITHOUT vsync (0 = immediate, no wait)
            g_Device.g_pSwapChain->Present(0, 0);
            
            // Frame limiter - cap at 120 FPS to prevent GPU thrashing
    ...
    bool OSImGui_External::CreateMyWindow()
    {
        ...
        // Note: If CreateWindowInBand doesn't exist, we'll fall back to CreateWindowEx below
...
//do not use uaicess for debugging/profiling (uiacess restarts the cheat)
#ifndef DBDEBUG
        // Try CreateWindowInBand first, fall back to normal CreateWindowEx
        if (pCreateWindowInBand) {
            constexpr DWORD exStyle = WS_EX_TOPMOST | WS_EX_NOACTIVATE | WS_EX_TRANSPARENT | WS_EX_LAYERED;
        ...
        // Fallback to normal CreateWindowEx if CreateWindowInBand failed or doesn't exist
        if (!Window.hWnd) {
            if (Type == ATTACH) {
