...
namespace DriverManager {
    ...
    inline bool ConnectOrLoadDriver(driver::DriverHandle& driver_handle, const VersionManager::VersionConfig& config) {
        ...
        // Always use the built-in device name; ignore config.deviceSymbolicLink
        if (!DriverLoaderHelper::LoadDriverWithKDU()) {
        ...
        for (int attempt = 1; attempt <= maxConnectionAttempts; attempt++) {
            ...
            // Recreate handle using the default device name (airway_radio)
